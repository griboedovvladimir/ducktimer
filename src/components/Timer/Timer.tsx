...
class Timer extends Component<any, any> {
  ...
  public componentDidUpdate(nextProps: any): void {
    if (
      this.props.time !== OTHER_CONSTANTS.START_TIME &&
      nextProps.time !== this.props.time
    ) {
      this.setState({
        ...this.state,
        currentTimerValue: this.props.time,
        timerFinished: false,
      });
      this.props.setTime({
        time: OTHER_CONSTANTS.START_TIME,
        id: this.props.id,
      });
    }
  }
      ...
      switch (event.target.name) {
        case 'hours':
          this.timerValue.hours =
            Number(event.target.value) > 23 ? '23' : event.target.value;
          break;
        case 'min':
  ...
  public onSetTimer = (): void => {
    const timerValue = `${this.timerValue.hours}:${this.timerValue.min}:${
      this.timerValue.sec
    }`;
    this.setState({
      ...this.state,
  ...
  public getFilmOptions = (): void => {
    if (!this.state.formIsActivated) {
      ...
      restService
        .post(API_CONSTANTS.FILM_FORM_FIRST_STEP, '')
        .then(response => response.json())
        .then(options => {
          this.filmFormActivate(
            <FilmPresetForm id={this.props.id} formsOptions={options} />
          );
        })
        .catch(() => {
  ...
  public getProcessOptions(): React.ReactNode[] {
    return SELECT_PROCESS_OPTIONS.map((process: string, i: number) => (
      <option key={i}>{process}</option>
    ));
  }
  ...
  render() {
    let timersClassList = this.state.timerFinished
      ? 'timers finished'
      : 'timers';
    let process =
      (this.state.otherProcess
        ? this.state.otherProcess
        : this.state.selectProcess) || 'default value';

    return (
      <div className={timersClassList}>
        <button onClick={this.onRemoveTimer} className="delete trans-color-btn">
          ×
        </button>
        <h4>{process}</h4>
        <div className="timer-panel">
          <span>
            {this.state.currentTimerValue || OTHER_CONSTANTS.START_TIME}
          </span>
          <p> Note: {this.state.note}</p>
          <button onClick={this.onStopTimer} title="pause" className="icon2">
            
          </button>
          <button onClick={this.onStartTimer} title="start" className="icon2">
            
          </button>
          <button onClick={this.onSetTimer} title="set" className="icon2">
            
          </button>
        </div>
        {this.state.panelIsOpen && (
          <div className="set-timer-panel" defaultValue=" ">
            ...
            Other process
            <input
              onChange={this.onChangeTimerParams}
              type="text"
              name="process"
              size={4}
