...
class SecondFilmPresetForm extends Component<
  ISecondFilmPresetFormProps,
  ISecondFilmPresetFormState
> {
  ...
  public setTime = () => {
    ...
    restService
      .post(API_CONSTANTS.GET_TIME_BY_PARAMS, {
        ...this.parametersModel,
        ...this.props.firstFormData,
      })
      ...
      .then(data => {
        if (data === 'false') {
          ...
          this.props.setTime({
            time: parseMinutsIntoReadableTime(data),
            id: this.props.id,
          });
        }
      });
  ...
  public render(): React.ReactNode {
    const dilution = this.props.formsOptions[0].map(dilution => (
      <option>{dilution}</option>
    ));
    const iso = this.props.formsOptions[1].map(iso => <option>{iso}</option>);
    const temp = this.props.formsOptions[2].map(temp => (
      <option>{temp}</option>
    ));
    return (
      <div>
