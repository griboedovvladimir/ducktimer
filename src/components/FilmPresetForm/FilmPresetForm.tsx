...
class FilmPresetForm extends Component<
  IFilmPresetFormProps,
  IFilmPresetFormState
> {
  ...
  public render(): React.ReactNode {
    const films = this.props.formsOptions[0].map(film => (
      <option>{film}</option>
    ));
    const developers = this.props.formsOptions[1].map(developer => (
    ...
    return (
      <form>
        ...
        <button
          ...
          onClick={this.getSecondFilmFormOptions}
          type="button"
        >
          Select
        </button>
